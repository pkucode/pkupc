...
def getcon(course):
    ...
    res=requests.get("http://pkupc.cn/programming/course/"+course+"/show.do")
...
def login(username,password,content,course):
    ...
    data={
    "referer": "http://pkupc.cn/programming/course/"+course+"/show.do",
          "username":username,
          "password":password,
          "login":"登录"}
    import requests
    sess = requests.Session()
    res=sess.post("http://pkupc.cn/programming/login.do",data=data,headers=header).headers
...
def pkupc(content,cookie,filename,course):
    ...
    ###########
    q = "http://pkupc.cn/programming/course/"+course+"/showProblemList.do?problemsId="
    ...
    header={'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'Accept-Encoding': 'gzip, deflate', 'Accept-Language': 'en-US,en;q=0.9', 'Connection': 'keep-alive',
            'Host': 'pkupc.cn',
            'Referer': 'http://pkupc.cn/programming/course/'+course+'/show.do?referer=http%3A%2F%2Fpkupc.cn%2Fprogramming%2Fcourse%2F'+course+'%2Fshow.do',
    ...
    for f in qq:
        ...
        quizhref = "http://pkupc.cn/programming/problem/"+f+"/show.do?problemsId="+content
        ...
        count=1
        for t in targe:
            target="http://pkupc.cn/programming/problem/solution.do?solutionId="+t+"&sourceCode=true"
